...
$(function(){
	...
	$('header .navigation').clone().prepend('<li><a href="/">Home</a></li>').wrap('<nav />').parent().attr('id', 'menu').mmenu({
		...
		searchfield: {add:true, search: false },
		navbar: {title: '' }
	}).find('.mm-search input').addClass('text').attr('name', 'q').parent().wrapInner('<form class="search" role="search" action="/search.aspx"></form>');;
